...
# biological_measure,BH_adjust_pvalue
# combine c1 and BH_cluster_CHM_table
c1$biological_category <- c1$biological_measure %>%
  gsub(pattern = "_.*$", replacement = "", perl = TRUE)
c1$BH_adjust_pvalue <- 
  BH_cluster_CHM_table$BH_adjust_pvalue[match(c1$biological_category,
                                    BH_cluster_CHM_table$biological_measure)]

# save
fwrite(c1,
      "../result/cluster_biomarker_tests_ratio_pvalue.csv") 
